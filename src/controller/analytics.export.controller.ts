...
/* ================= IMAGE FETCH HELPER (NO node-fetch) ================= */
...
export const exportAnalyticsPDF = async (req: AuthRequest, res: Response) => {
  try {
    ...
    transactions.forEach(t => {
      const catName =
        t.category_id ? categoryNameMap[t.category_id.toString()] ?? "Other" : "Other"
    ...
    const barBuffer = await fetchImageBuffer(await barChart.getShortUrl())
    ...
    const pieBuffer = await fetchImageBuffer(await pieChart.getShortUrl())
    ...
    doc.image(barBuffer, 50, doc.y + 20, { width: doc.page.width - 100, height: 220 })

    doc.addPage()
    sectionTitle(doc, "Category-wise Expenses")
    doc.image(pieBuffer, (doc.page.width - 400) / 2, doc.y + 10, { width: 400, height: 280 })
